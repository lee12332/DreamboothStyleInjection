import jittor as jt
import numpy as np, copy, os, sys, json
# import matplotlib.pyplot as plt

from styleID.utils import *  # image save utils

from styleID.stable_diffusion import *  # load SD
import copy
...
from styleID.config import get_args

message = {
    "00": {"pre": "a ", "post": " neon line as texture"},
    "01": {"pre": "a ", "post": " light oil painting depiction"},
    "02": {"pre": "a ", "post": " Made_of_Clay"},
    "03": {"pre": "a ", "post": " with pencil drawing style"},
    "04": {"pre": "a ", "post": ""},
    "05": {"pre": "a ", "post": " slight chaotic curve background"},
    "06": {"pre": "a ", "post": " red frame, hollow out"},
    "07": {"pre": "a ", "post": " on the street"},
    "08": {"pre": "a ", "post": " made of paper"},
    "09": {"pre": "a ", "post": " watercolor style"},
    "10": {"pre": "", "post": ",Colorful background"},
    "11": {"pre": "", "post": ",Solid color background"},
    "12": {"pre": "", "post": ",made_of_lego"},
    "13": {"pre": "", "post": ",ink_painting"},
    "14": {"pre": "a ", "post": ",made_of_ink_line"},
    "15": {"pre": "", "post": ",Colorful background"},
    "16": {"pre": "a ", "post": ",slight_pixel_art_depiction"},
    "17": {"pre": "", "post": ",watercolor"},
    "18": {"pre": "", "post": ",Solid color background,Colored pencil painting"},
    "19": {"pre": "a ", "post": ",made_of_little_blocks"},
    "20": {"pre": "a ", "post": ",within_a_circular"},
    "21": {"pre": "a ", "post": ",layered_paper_art_design"},
    "22": {"pre": "a ", "post": ",color_lead_painting"},
    "23": {"pre": "a ", "post": ",pen_drawing"},
    "24": {"pre": "a ", "post": ",made_of_gummies"},
    "25": {"pre": "a ", "post": ",pixel_style"},
    "26": {"pre": "a ", "post": ",made_of_paper"},
    "27": {"pre": "a ", "post": ",bottom_is_lake,background_are_mountains"},
}
...
class style_transfer_module:
    ...
    def __init__(
        ...
        self.trigger_modify_qkv = (
            False  # if set True --> save attn qkv by self.attn_features_modify
        )
    ...
    def get_text_condition(self, text):
        if text is None:
            ...
            uncond_embeddings = text_encoder(uncond_input.input_ids.to(device))[0].to(
                device
            )
        ...
        denoise_kwargs = {"encoder_hidden_states": jt.concat(text_cond)}
    ...
    def reverse_process(self, input, denoise_kwargs):
        ...
        for t in tqdm(self.scheduler.timesteps):
            ...
            with jt.no_grad():
                ...
                noisy_residual = unet_wrapper.unet(
                ...
                ).prev_sample  # coef * P_t(e_t(x_t)) + D_t(e_t(x_t))
                pred_original_sample = self.scheduler.step(
                    noisy_residual, t, input
                ).pred_original_sample  # D_t(e_t(x_t))
...
if __name__ == "__main__":
    ...
    vae, tokenizer, text_encoder, unet, scheduler = load_stable_diffusion(
        sd_version=str(cfg.sd_version), precision_t=dtype
    )
    ...
    unet_wrapper = style_transfer_module(
        ...
        tokenizer,
        scheduler,
        style_transfer_params=style_transfer_params,
    )
    ...
    # Get seed Number
    with open("./seed.json", "r") as file_seed:
        # 加载JSON数据
        seed_data = json.load(file_seed)
    seed_number = seed_data["seed"]

    os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"
    min_num = cfg.style_item
    max_num = cfg.style_item + 1
    # dataset_root = "/root/autodl-tmp/B"
    dataset_root = cfg.sty_fn
    lora_weights = "./ckpt/"
    generator = jt.set_global_seed(seed_number)

    for i in range(min_num, max_num):
        print("第" + str(i) + "风格==============")
        sty_path = cfg.sty_fn + "/{:0>2d}/images/".format(i)
        files = [
            os.path.join(sty_path, f)
            for f in os.listdir(sty_path)
            if os.path.isfile(os.path.join(sty_path, f))
        ]

        cnt_path = cfg.save_dir + "/{:0>2d}/".format(i)
        ...
        for id, prompt in prompts.items():
            ...
            if int(id) / 8 == sty_tag:
                ...
                unet_wrapper.trigger_get_qkv = (
                    True  # get attention features (key, value)
                )
                unet_wrapper.trigger_modify_qkv = False

                style_latent = encode_latent(
                    normalize(style_image).to(device=vae.device, dtype=dtype), vae
                )

                # invert process
                print("Invert style image...")
                images, latents = unet_wrapper.invert_process(
            ...
            cnt_img_path = f"{cnt_path}/{prompt}.png"
            image.save(f"{cnt_path}/{prompt}.png")
            content_image = cv2.imread(cnt_img_path)[:, :, ::-1]
            ...
            content_latent = encode_latent(
                normalize(content_image).to(device=vae.device, dtype=dtype), vae
            )
            ...
            for layer_name in style_features.keys():
                ...
                for t in scheduler.timesteps:
                    ...
                    unet_wrapper.attn_features_modify[layer_name][t] = (
                        ...
                        style_features[layer_name][t][1],
                        style_features[layer_name][t][2],
                    )  # content as q / style as kv
            # =============================================

            unet_wrapper.trigger_get_qkv = False
            unet_wrapper.trigger_modify_qkv = (
                not cfg.without_attn_injection
            )  # modify attn feature (key value)
            ...
            if cfg.without_init_adain:
                ...
                mean_content = content_latent.mean(dim=3, keepdim=True).mean(
                    dim=2, keepdim=True
                )
                ...
                mean_style = style_latent.mean(dim=3, keepdim=True).mean(
                    dim=2, keepdim=True
                )
                # 计算 latent_cs
                latent_cs = (content_latent - mean_content) / (
                    std_content + 1e-6
                ) * std_style + mean_style
            ...
            save_image(image_last, os.path.join(save_dir, prompt + ".png"))
